...
void						pixel_bigwidth(t_doom *den, int x, int y,
														int colort);
Uint32						get_pixel2(SDL_Surface *sur, const int x,
														const int y);
void						coordinate_network(int x, int y, t_doom *den,
														t_col *vec);
void						set_pixel(SDL_Surface *bmp, int x, int y,
														Uint32 pixel);
...
void						ft_lstnewmap(t_wals **new, int x, int y,
															t_doom *den);
void						ft_lstnewmap_sec(t_wals **new, int x, int y,
															t_doom *den);
void						ft_save_parm(t_wals **tmp, int x, int y,
															t_doom *den);
void						ft_lstnewmap_sec_help(t_doom *den);
float						isleft_or_right(t_wals *list);
void						sort_sector(t_doom *den);
void						add_sector_dir(t_wals *tmp, t_wals *last,
															t_sort *sort);
...
void						write_list_help(t_doom *den, t_wals *tmp,
													t_wals *rmp, t_wals *nmp);
...
void						sector_in_sector_two(t_doom *den, t_wals *cmp,
													t_wals *lmp);
void						sector_in_sector(t_doom *den);
void						sort_walls_inside(t_doom *den);
int							inside_sect(t_doom *den, t_wals **tmp, int sec);
void						sort_walls_inside_help(t_doom *den, int *walls,
													int i, t_wals *rmp);
int							go_delete(int sec, t_wals *rmp, t_doom *den,
													int k);
int							go_delete_help(int sec, t_wals *rmp,
													t_doom *den, int k);
...
void						delete_sector_memory_one(t_doom *den,
													t_sector_list *first);
void						delete_sector_memory_two(t_doom *den,
													t_sector_list *first);
...
void						make_portal_sector2(t_doom *den,
							t_wals *first_element, t_wals *first_element_new);
void						portal_sector_new(t_doom *den, t_wals *rmp,
															t_wals *lmp);
...
void						sort_help(int i, t_wals *rmp, t_wals *tmp,
														int *walls);
...
void						next_wall_change(t_doom *den, t_wals *tmp,
													t_wals *rmp);
void						next_wall (t_doom *den);
void						back_wall_check(t_doom *den, t_wals *rmp,
													t_wals *lmp);
...
void						info_display_two(t_doom *den, char *infor,
													SDL_Color color_text);
void						info_display_three(t_doom *den, char *infor,
													SDL_Color color_text);
void						change_wall_two(t_doom *den, SDL_Color color_text,
													char *infor);
...
void						mouse_button_down_sdl(t_doom *den, t_col *vec,
														SDL_Event e);
void						mouse_button_down_up_sdl(t_doom *den, t_col *vec,
														SDL_Event e);
void						key_down_sdl(t_doom *den, t_col *vec,
														const Uint8 *state);
void						ft_sdl_init(t_doom *den);
void						mouse_button_down(t_doom *den, t_col *vec,
														int x, int y);
...
void						mouse_button_down_select(t_doom *den,
													t_col *vec, SDL_Event e);
void						move_scan_sector(t_doom *den, const Uint8 *state);
void						move_scan_sprite(t_doom *den, const Uint8 *state);
void						move_scancode(t_doom *den, t_col *vec,
												const Uint8 *state);
void						mouse_button_rotate(t_doom *den, t_col *vec,
												int x, int y);
void						mouse_button_sprite(t_doom *den, t_col *vec,
												int x, int y);
int							count_walls(t_wals *tmp, t_getmap *gm,
												t_sdl *iw, t_doom *den);
...
void						unget_map_help(t_sdl *iw, t_wals *tmp,
												t_sector_list *sl);
...
void						save_reverse_map_wall_animations(t_doom *den,
